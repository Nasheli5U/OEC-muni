        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4">{{ $expediente->nombres }}</td>
                    ...
                    <td class="px-6 py-4">{{ $expediente->monto }}</td>
                    <td class="px-6 py-4">{{ $expediente->estado }}</td>
                    <td class="px-6 py-4 flex space-x-2">
                        <!-- Botones de acción -->
                        <form action="{{ route('expedientes.index', $expediente->id) }}" method="GET">
                            <button type="submit" class="btn-editar">
                                Editar
                            </button>
                        </form>
                        <form action="{{ route('expedientes.index', $expediente->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
